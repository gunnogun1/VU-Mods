Events:Subscribe('Level:Loaded', function()
	...
	for i,weapon in pairs(CustomizationUnlockParts(CustomizationTable(ruSupport.weaponTable).unlockParts[1]).selectableUnlocks) do 
		if i <= 11 then
	...
	CustomizationUnlockParts(CustomizationTable(ruRecon.weaponTable).unlockParts[1]):MakeWritable()
	for i,weapon in pairs(CustomizationUnlockParts(CustomizationTable(ruSupport.weaponTable).unlockParts[1]).selectableUnlocks) do 
		if i <= 11 then
			--print("Changed Recon")
		CustomizationUnlockParts(CustomizationTable(ruRecon.weaponTable).unlockParts[1]).selectableUnlocks:add(weapon)
		end
	end
	...
	for i,weapon in pairs(CustomizationUnlockParts(CustomizationTable(usSupport.weaponTable).unlockParts[1]).selectableUnlocks) do
		if i <= 11 then
			--print("Changed Assault")
	...
	for i,weapon in pairs(CustomizationUnlockParts(CustomizationTable(usSupport.weaponTable).unlockParts[1]).selectableUnlocks) do
        ...
        CustomizationUnlockParts(CustomizationTable(usRecon.weaponTable).unlockParts[1]):MakeWritable()
	for i,weapon in pairs(CustomizationUnlockParts(CustomizationTable(usSupport.weaponTable).unlockParts[1]).selectableUnlocks) do
		if i <= 11 then
			--print("Changed Recon")
		CustomizationUnlockParts(CustomizationTable(usRecon.weaponTable).unlockParts[1]).selectableUnlocks:add(weapon)
		end
	end
